...
if __name__ == "__main__":
    assert len(sys.argv) == 4, f"USage: python script.py tags_and_clusters.csv tags_and_quotes.csv model"
    ...
    theme_dict = {}
    with tqdm(total=len(clusters_to_tags), desc="Processing themes") as pbar:
    ...
    theme_titles = []
    tag_quotes = []
    with tqdm(total=len(data), desc="Processing themes") as pbar:
